    ...
    <div className="max-w-3xl mx-auto px-6 pt-28 pb-16">
      <div className="mb-6">
        <Badge variant={e.is_upcoming ? 'default' : 'secondary'} className="mb-3">
          {e.is_upcoming ? 'Kommend' : 'Vergangene Veranstaltung'}
        </Badge>
      ...
      {e.recap_body && (
        <div className="prose prose-sm dark:prose-invert max-w-none">
          <h2 className="text-lg font-semibold mb-4">Veranstaltungsrückblick</h2>
      ...
      {e.recap_images.length > 0 && (
        <div className="mt-8 grid grid-cols-2 gap-3">
          {e.recap_images.map((url, i) => (
            <img key={i} src={url} alt={`Veranstaltungsfoto ${i + 1}`} className="rounded-xl w-full object-cover aspect-video" />
          ))}
        </div>
      )}
    </div>
